...
def knapsack_value(volume_and_values: list[tuple[int, int]], chosen_objects: list[int]) -> int:
    value = 0
...
def pack_knapsack(max_knapsack_volume: int, volume_and_values: list[tuple[int, int]]) -> list[int]:
    best_knapsack = [[]]
    ...
    for knapsack_volume in range(1, max_knapsack_volume + 1):
        ...
        for idx, volume_and_value in enumerate(volume_and_values):
            ...
            new_knapsack_value = value + knapsack_value(volume_and_values, smaller_knapsack)
            if new_knapsack_value > best_knapsack_value:
                best_knapsack[knapsack_volume] = best_knapsack[knapsack_volume - volume] + [idx]
